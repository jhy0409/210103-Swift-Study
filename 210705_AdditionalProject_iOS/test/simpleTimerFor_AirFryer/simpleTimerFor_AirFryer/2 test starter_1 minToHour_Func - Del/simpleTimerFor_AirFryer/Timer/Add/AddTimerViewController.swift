...
class AddTimerViewController: UIViewController, UITextFieldDelegate {
    ...
    @IBAction func addButtonTap(_ sender: Any) {
        ...
        let minToHour : (h: String, m: String)? = minToHour(Int(hour)!, Int(min)!)
        guard let h = minToHour?.h, let m = minToHour?.m else { return }
        hour = h; min = m
    ...
    func minToHour(_ hour: Int, _ min: Int) -> (String, String)? {
        if min > 60 && hour == 0  {
            let localH = String(min / 60) // 60으로 나눈 몫
            let localM = String(min % 60) // 60으로 나눈 나머지
            return (localH, localM)
        }
        
        else { print("-----> func minToHour"); return nil }
    }
}
