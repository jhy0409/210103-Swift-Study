...
extension EditTimerViewController {
    ...
    @IBAction func editCurrentFood(_ sender: Any) {
        ...
        if tOrF == true {
            ...
            btnSenderTxt = "NONE"
            showAlert("타이머 추가 완료") {
                guard let editVC = self.presentingViewController else { return }
                self.dismiss(animated: true) {
                    editVC.present(AFTimerViewController(), animated: true, completion: {AFTimerViewController().foodViewModel.loadFoods()})
                }
            }
        }
    }
...
extension EditTimerViewController {
    ...
    func showAlert(_ strMsg: String, _ completion: @escaping () -> ()) {
        let alertController = UIAlertController(title: "확인", message: strMsg, preferredStyle: .alert)
