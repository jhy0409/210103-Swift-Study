...
class AddTimerViewController: UIViewController, UITextFieldDelegate {
    ...
    @IBAction func addButtonTap(_ sender: Any) {
        ...
        // [x] 유효값 확인목록 튜플로 저장
        let (if1_hourNMinZero, if2_hourZero, if3_foodNameEmpty, if4_ondoZero, if5_minToH_T, if6_maximumPerDay) =
            ((hour == "0" && min == "0"), (hour != "0" && Int(min)! > 60 ), foodName.isEmpty,
             ondo == "0", (hour == "0" && Int(min)! > 60 ), foodViewModel.manager.maximumPerDay <= 0)
        // [x] 유효값 검사 후 반환값이 true일 때만 아래코드 실행
        let tOrF = showAlert(if1_hourNMinZero, if2_hourZero, if3_foodNameEmpty, if4_ondoZero, if6_maximumPerDay)
        if tOrF == true {
...
extension AddTimerViewController {
    func showAlert(_ if1_hourNMinZero: Bool, _ if2_hourZero: Bool, _ if3_foodNameEmpty: Bool, _ if4_ondoZero: Bool, _ if6_maximumPerDay: Bool) -> Bool {
        if (if1_hourNMinZero == true || if2_hourZero == true || if3_foodNameEmpty == true || if4_ondoZero == true || if6_maximumPerDay == true) {
            var str = String()
            ...
            if if6_maximumPerDay == true { str.append("- 하루생성 개수를 초과하였습니다.\n") }
