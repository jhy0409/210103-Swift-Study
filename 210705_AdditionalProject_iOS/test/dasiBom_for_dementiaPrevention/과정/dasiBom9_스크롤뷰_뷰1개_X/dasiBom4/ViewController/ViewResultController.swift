...
class ViewResultController: UIViewController {
    ...
    @IBOutlet weak var contentViewInScroll: UIView!
    
    var toptableVIewViewHeight: CGFloat {
        toptableVIew.layoutIfNeeded()
        return toptableVIew.contentSize.height
    }
    
    var bottomTableViewHeight: CGFloat {
        bottomTableView.layoutIfNeeded()
        return bottomTableView.contentSize.height
    }
    
    var totaltableHeight: CGFloat {
        return toptableVIewViewHeight + bottomTableViewHeight * 20
    }
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if expertise.count > 0, getUserGameInfoArr.count > 0, getUserTestInfoArr.count > 0 {
            ...
            print("\n\n\n viewWillAppear ---> totaltableHeight : \(totaltableHeight)")
            contentViewInScroll.frame.size.height = totaltableHeight
            
            self.view.frame = CGRect(x: 0, y: 0, width: self.view.frame.width, height: totaltableHeight)
            
            
            toptableVIew.frame.size.height = toptableVIewViewHeight
            bottomTableView.frame.size.height = bottomTableViewHeight
